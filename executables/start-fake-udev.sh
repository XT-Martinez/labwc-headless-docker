...
stdbuf -oL udevadm monitor --kernel --subsystem-match=input --property | while IFS= read -r line; do
    ...
    if [[ -z "$line" || "$line" =~ ^KERNEL ]]; then
        ...
        if [[ ${#event_props[@]} -gt 0 ]]; then
            ...
            if [[ -v event_props["ACTION"] && \
                  ...
                  ( "${event_props["DEVNAME"]}" == /dev/input/event* || "${event_props["DEVNAME"]}" == /dev/input/js* ) && \
                  -v event_props["MAJOR"] && \
                  -v event_props["MINOR"] && \
                ...
                fake_mac_addr=$(generate_random_mac)
                uniq="${event_props["UNIQ"]:-$fake_mac_addr}"

                # Look up the parent's name based on DEVPATH
                if [[ -v parent_device_names["$parent_path"] ]]; then
                    device_type="${parent_device_names["$parent_path"]}"
                    log_debug "Device node $devname corresponds to parent '$device_type' ($parent_path)"
                else
                    # Check if the event itself has a name we target directly (less likely for event/js nodes)
                    event_name=""
                    ...
                    if contains_element "$event_name" "${TARGET_PARENT_DEVICE_NAMES[@]}"; then
                         ...
                         log_debug "Device node $devname does not correspond to a known parent DEVPATH '$parent_path' and its own name '$event_name' is not targeted. Skipping."
                         event_props=()
                         continue
                    fi
                fi
                ...
                if [[ "$action" == "add" ]]; then
                    ...
                    db_content=""
                    fake_udev_payload=""
                    # Determine generator based on parent type AND if it's event* or js*
                    # Generally, use the same logical type (kbd, mouse, gamepad) for both
                    case "$device_type" in
                        ...
                        "Sunshine Nintendo (virtual) pad"|"Sunshine X-Box One (virtual) pad")
                            # Use gamepad generators for both event* and js* nodes
                    ...
                    remove_payload=$(echo "ACTION=remove\0DEVNAME=$devname\0NAME=$device_type\0MAJOR=$major\0MINOR=$minor\0SUBSYSTEM=input\0DEVPATH=$devpath\0SEQNUM=${seqnum_orig}\0")
